...
def processFeedback(feedback):
    ...
    if feedback.event_type == InteractiveMarkerFeedback.MENU_SELECT:
        if feedback.menu_entry_id == 1:
            ...
            listener.waitForTransform('/llink2', '/base_link', rospy.Time(), rospy.Duration(1.0))
            (trans, rot) = listener.lookupTransform('base_link', 'llink2', rospy.Time())
            print(trans, rot)
...
if __name__=="__main__":
	...
	print (listener.frameExists('llink2'))
	(trans,rot)=listener.lookupTransform('base_link','llink2',rospy.Time())
	print (trans,rot)
